...
test("selectTests: direct matches skip Jev, others are judged and cached", async () => {
  ...
  const cache = {};
  // Fork API: the backend is explicit (scoreRows pattern) and the explicit apiKey
  // keeps the run hermetic — no key-file lookup, no resolveApiKey call.
  const r = await selectTests(diff, tests, { threshold: 0.5, batch: 16, concurrency: 2, apiKey: "k", backend: BACKENDS.typesafe, fetchImpl, cache });
  ...
  expect(calls[0].model).toBe("jev-latest"); // the resolved model rides in the payload
  expect(r.selected.map((s) => [s.file, s.reason])).toEqual([["src/rows.test.ts", "direct"], ["src/cli.test.ts", "jev"]]);
  const r2 = await selectTests(diff, tests, { threshold: 0.5, batch: 16, concurrency: 2, apiKey: "k", backend: BACKENDS.typesafe, fetchImpl, cache });
